...
> **0 篇的日子不提交任何文件**。运行记录看 Routine 详情页即可。
...
if [ "$N" -eq 0 ]; then
  echo "No items today—skipping commit. 检查 Routine 详情页查运行日志。"
  exit 0
fi

git add archive/$TODAY
git commit -m "archive: $TODAY · $N items"
...
- 有报道：今天抓了几篇、分别什么主题、是否有需要人工核实的（垂直源 / 合成稿）。
- 0 篇：仅说一句"今日白名单源未发布相关内容"即可，不提交任何文件。
...
- **0 篇不提交**——运行记录看 Routine 详情页，仓库只留有价值的报道。
- 翻译卡壳的句子用 `[原文]` 标出来不要硬翻。
- 图片下载脚本失败不要让全篇失败 —— 记录 URL，正文继续。
- 时间一律 UTC；用 `date -u` 不要用 local time。
- **Push 必须走 `git push origin HEAD:main`**：会话本身在 `claude/<random>` 临时分支，归档统一进 main。
